...
void RTreeInitRect(struct Rect *R) {
  register struct Rect *r = R; // ed. the rect struct just has an array of float
...
struct Rect RTreeNullRect() {
  ...
  r.boundary[0] = (RectReal)1;        // ed. first low side at 1
  r.boundary[NUMDIMS] = (RectReal)-1; // ed. opposite side with index NUMDIMS
  for (i = 1; i < NUMDIMS; i++)
    r.boundary[i] = r.boundary[i + NUMDIMS] =
        (RectReal)0; // ed. all intermediates coordinates at 0
  return r;
}
...
void RTreePrintRect(struct Rect *R, int depth) {
  ...
  RTreeTabIn(depth); // ed. just to put a number of tabs depending of depth
                     // (defined line 63 in NODE.C)
  printf("rect:\n");
  for (i = 0; i < NUMDIMS; i++) {
    RTreeTabIn(depth + 1);
    printf("%f\t%f\n", r->boundary[i],
           r->boundary[i + NUMDIMS]); // ed. xmin, xmax and then ymin and ymax
    // ed. to indicate the sides (xmin to xmax and then ymin to ymax), which is
    // more intuitive to represent a rectangle
  }
}
...
int RTreeOverlap(struct Rect *R, struct Rect *S) {
  ...
  for (i = 0; i < NUMDIMS; i++) {
    j = i + NUMDIMS; /* index for high sides */ // ed.  j is the index for the
                                                // max and i for the min
    if (r->boundary[i] > s->boundary[j] || s->boundary[i] > r->boundary[j]) {
      return FALSE;
    }
    /* Ed.
    We don't prove that they overlap, we prove the opposite!
    For two rectangles to overlap, they need to overlap in all dimensions.
    Hence to see if they don't overlap, we can check if they do so in at least
    one dimension. (And return FALSE)

    To be more precise :
    (r->boundary[i] > s->boundary[j])
    == "If the left size of rectangle r is a the right of rectangle s" then no
    overlap (s->boundary[i] > r->boundary[j])
    == "If the left size of rectangle s is a the right of rectangle r" then no
    overlap"

    So if they are not separated, then they overlap.
    (And return TRUE)
    */
  }
  return TRUE;
...
int RTreeContained(struct Rect *R, struct Rect *S) {
  ...
  /* Ed.
  Here we use the same idea as RTreeOverlap(). For a rectangle r to be contained
  in s, In both dimensions it need to be inside s. In all dimensions : Its
  inferior side (meaning, the minimum) has to be superior or equal to the
  inferior side of s. Its superior side has to be inferior or equal to the
  superior side of s.
  */
  return result;
}
